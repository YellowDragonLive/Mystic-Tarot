...
const SpreadLayout: React.FC<SpreadLayoutProps> = ({ spreadId, positions, drawnCards, onCardClick }) => {
  ...
  const renderSlot = (index: number, className: string = "") => {
    ...
    return (
      <div key={positionData.id} className={`relative flex flex-col items-center ${className}`}>
        <div className="relative w-24 h-40 sm:w-32 sm:h-52 md:w-40 md:h-64">
           {/* Placeholder Outline */}
           {!drawn && (
        ...
        {/* Label below card */}
        <div className="mt-2 text-xs sm:text-sm text-amber-200/70 font-serif text-center max-w-[100px]">
            {positionData.name}
        </div>
      </div>
    );
  ...
  if (spreadId === 'timeflow') {
    return (
      <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-8 h-full min-h-[50vh]">
        {renderSlot(0)}
        {renderSlot(1, "mt-0 sm:-mt-8")} {/* Center card slightly elevated visually */}
        {renderSlot(2)}
      </div>
    );
  }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto p-4">
        <div className="flex flex-col md:flex-row justify-center gap-8 md:gap-16 items-center">
            ...
            {/* Left Cluster: Cross */}
            <div className="relative w-[300px] h-[300px] sm:w-[350px] sm:h-[450px] flex items-center justify-center">
